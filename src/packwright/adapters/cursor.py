...
from packwright.core.handoff import handoff_feature, target_handoff_artifacts
...
def compile_to_cursor_pack(mechanism, references=None):
    """Compile a resolved character mechanism into a Cursor adapter pack."""
    validate_mechanism(mechanism)
    if ADAPTER_NAME not in mechanism["targets"].get("supported", []):
        raise PackwrightValidationError(["Cursor adapter is not listed in targets.supported"])

    references = references or {}
    slug = character_slug(mechanism)
    main_rule_path = f".cursor/rules/{slug}.mdc"
    memory_rule_path = f".cursor/rules/{slug}-memory.mdc"
    save_context_path = save_context_skill_path(mechanism, ADAPTER_NAME)
    pack = {
        ...
        save_context_path: _render_save_context_rule(mechanism, references),
    }
    pack.update(target_handoff_artifacts())
...
def _render_main_rule(mechanism, memory_rule_path, save_context_path):
    ...
    lines = [
        "---",
        f"description: Core behavior and working rules for {name}.",
        "alwaysApply: true",
        "---",
        "",
        f"# {name}",
    ...
    lines.extend(
        [
            ...
            "",
            "## Use When Needed",
            f"- Use `{save_context_path}` for milestone handoff, session close, or explicit context-save requests.",
            "- Use `scripts/handoff_export.sh` for real cross-agent or cross-runtime handoff files.",
            "- Use `workspace/shared/artifacts/handoffs/` for real cross-agent or cross-runtime handoff files.",
            "- Use `workspace/shared/artifacts/session-briefs/` for same-agent next-session briefs; do not label them handoffs.",
            f"- Use `{memory_rule_path}` when prior context, domain routing, project state, or source lookup may matter.",
...
def _render_memory_rule(mechanism):
    name = character_name(mechanism)
    lines = [
        "---",
        f"description: Memory loading and ownership rules for {name}. Use when prior context may matter.",
        "alwaysApply: false",
        "---",
        "",
        f"# {name} Memory Contract",
        "",
        "Use this rule when a request needs prior project, domain, source, todo, session, or collaboration context.",
        "",
        "## Loading Policy",
        "- Read `memory/index.md` first; it routes to the owner file for each kind of memory.",
        "- Load only the relevant owner files for the current task.",
        "- Prefer source-map pointers and source files when factual accuracy matters.",
        "- Read `knowledge/index.md` only for reviewed reusable domain knowledge; do not treat it as current project state.",
        "- Use `sources/*/manifest.json` for provenance lookup when source evidence matters.",
        "- Do not load compatibility files by default unless the memory index points to them.",
        "- Keep generated work products in `workspace/<domain>/`, not in memory files.",
        "",
        "## Core Files",
        "- `memory/profile.md`: explicit stable facts and preferences.",
        "- `memory/workstreams.md`: long-running domain routing.",
        "- `memory/workstreams/<slug>.md`: dense domain state.",
        "- `memory/projects/<slug>.md`: project state, decisions, and open loops.",
        "- `memory/session-index.md`: earlier session lookup.",
        "- `memory/source-map.md`: source, account, file, and artifact lookup.",
        "- `memory/todos.md`: action queues and commitments.",
        "- `memory/collaboration.md`: stable collaboration calibration.",
        "",
    ]
    return "\n".join(lines)
...
def _render_save_context_rule(mechanism, references):
    ...
    lines.extend(
        [
            ...
            "- `.emotion-engine/codex-state.json` stores dynamic emotion state when enabled; do not mirror it into memory files.",
...
def _render_manifest(mechanism, references, artifacts):
    ...
    manifest = {
        ...
        "features": {
            "emotion_engine": emotion_engine_feature("cursor", installed=False, mode=emotion_mode),
            "memory": _memory_feature(),
            "knowledge": knowledge_feature(),
            "workspace": workspace_feature(),
            "handoff": handoff_feature(),
            "cursor_rules": {
                "rules_dir": ".cursor/rules",
                "format": "mdc",
                "main_rule": f".cursor/rules/{slug}.mdc",
                "memory_rule": f".cursor/rules/{slug}-memory.mdc",
                "save_context_rule": save_context_skill_path(mechanism, ADAPTER_NAME),
            },
        },
        "local_tools": {
            "handoff_export": handoff_feature(),
        },
        ...
        "boundaries": {
            ...
            "emotion_engine_runtime": "spec_guided_behavior_only",
