...
def _render_main_rule(mechanism, memory_rule_path, save_context_path):
    ...
    extra_skill_lines = [
        f"- Use `{record['path']}` when {skill_spec(mechanism, record['id'])['trigger']}"
    ...
    route_index = lines.index("- Use `scripts/handoff_export.sh` for real cross-agent or cross-runtime handoff files.")
    lines[route_index:route_index] = extra_skill_lines
