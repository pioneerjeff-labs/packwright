...
For an existing local agent, begin with `packwright adopt --from <source-dir> --dry-run`. Show the inventory and review summary. If I ask to create review materials, run `packwright adopt --from <source-dir> --target <target-dir>` and show me the generated `adoption-review.yaml`. Its decisions begin as `pending`; review them with me one by one. This release does not apply the queue, merge memory, or import files automatically.
