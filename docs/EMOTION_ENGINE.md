...
- `.packwright/runtime/emotion-engine/generations/2.0.0-rc.4-400bd2e/` — the pinned helper, MCP server, cohort-aware launcher, schema, template, registration helper, projection receipt, and license;
- `.emotion-engine/generations/2.0.0-rc.4-400bd2e/state.json` — the rc.4 generation's live state;
...
- `.emotion-engine/generations/2.0.0-rc.4-400bd2e/state.json`
- `.emotion-engine/state.json`
- `.emotion-engine/codex-state.json`
- `.emotion-engine/emotion-state.json`

One valid legacy state is copied byte-for-byte into the current generation and retained at its old path. Existing generation state is never regenerated by install, refresh, `--force`, or `doctor --fix`. An old MCP process therefore keeps writing only its legacy path and cannot downgrade the current v3 packet. If multiple candidates contain different bytes, Packwright refuses to write until the ambiguity is resolved explicitly.
...
Refresh, migration, lifecycle events, and MCP requests share one target lock.
