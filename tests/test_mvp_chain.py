...
from packwright.core.install import (
    _emotion_legacy_retirement_plan,
    _emotion_engine_state_issue_for_path,
...
class MvpChainTest(unittest.TestCase):
    ...
    def test_projection_neutrality_allows_words_that_contain_pi(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            source_dir = Path(tmpdir) / "source"
            shutil.copytree(MECHANISM_PATH.parent, source_dir)
            skill_path = source_dir / "skills" / "save-context" / "SKILL.md"
            skill_path.write_text(
                skill_path.read_text(encoding="utf-8")
                + "\n- Use Pinpoint analysis and a Picture when they clarify the work.\n",
                encoding="utf-8",
            )
            resolved = resolve_mechanism(load_mechanism(source_dir / "mechanism.yaml"))
            pack = compile_to_codex_pack(resolved)
            ...
            self.assertTrue(neutrality["passed"], neutrality)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for locale, case in locale_cases.items():
                ...
                for index, from_adapter in enumerate(adapters):
                    ...
                    migrated = migrate_target(
                        installed[from_adapter],
                        target,
                        to_adapter=to_adapter,
                        accept_degraded=to_adapter == "cursor",
                    )
...
""",
            ...
            state = json.loads((target_dir / EMOTION_ENGINE_STATE_PATH).read_text(encoding="utf-8"))
...
""",
    ...
    def test_invalid_existing_emotion_state_is_rejected_without_mutation(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            state_file = Path(tmpdir) / "state.json"
            state_file.write_text("{bad json", encoding="utf-8")
            before = state_file.read_bytes()
            issue = _emotion_engine_state_issue_for_path(state_file)
            self.assertEqual(issue["id"], "emotion_engine_state_invalid")
    ...
    def test_migration_with_live_memory_keeps_a_clean_installed_score(self):
        resolved = resolve_mechanism(load_mechanism(MECHANISM_PATH))
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            pack_dir = root / "codex-pack"
            source_target = root / "codex-target"
            destination = root / "claude-target"
            _write_pack(
            ...
            install_pack(pack_dir, source_target)
            live_memory = {
                "memory/pinned.md": "# Pinned Memory\n\n## Pins\n\n- Keep receipts honest.\n",
                "memory/recent-activity.md": (
                    "# Recent Activity\n\n"
                    "Keep the newest 20 milestone pickup entries here.\n\n"
                    "<!-- entries -->\n\n"
            ...
            self.assertEqual(plan.to_dict()["score"]["planned"]["score"], 100.0)
    ...
    def test_install_dry_run_reports_stale_removal_without_writing(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            provenance_path = target_dir / ".packwright" / "install-provenance.json"
    ...
    def test_install_pack_can_explicitly_include_light_emotion_engine_for_codex(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(
                (target_dir / EMOTION_ENGINE_RUNTIME_ROOT / "scripts" / "emotion_engine_mcp.py").exists()
            )
            self.assertTrue(
                (target_dir / EMOTION_ENGINE_RUNTIME_ROOT / "scripts" / "register_mcp_client.py").exists()
            )
            self.assertTrue((target_dir / EMOTION_ENGINE_STATE_PATH).exists())
            self.assertTrue(sidecar["state_created"])
            self.assertTrue(sidecar["entry_updated"])
            self.assertEqual(sidecar["version"], EMOTION_ENGINE_VERSION)
            ...
            self.assertEqual(state["identity"]["character_id"], "atlas")
            self.assertEqual(state["identity"]["relationship_id"], "atlas:primary-user")
            self.assertIn("session_idempotency/v1", state["capabilities"])
            ...
            self.assertIn(f"{EMOTION_ENGINE_RUNTIME_ROOT}/scripts/emotion_engine_mcp.py", target_manifest["artifacts"])
            self.assertIn(f"{EMOTION_ENGINE_RUNTIME_ROOT}/scripts/register_mcp_client.py", target_manifest["artifacts"])
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for adapter, compiler in compilers.items():
                with self.subTest(adapter=adapter):
                    ...
                    self.assertEqual((target_dir / EMOTION_ENGINE_STATE_PATH).read_bytes(), legacy_state)
                    ...
                    self.assertEqual(mcp_status.returncode, 7, mcp_status.stderr)
                    self.assertEqual(
                        json.loads(mcp_status.stdout)["error"]["data"]["reason"],
                        "activation_state_verification_failed",
                    )
    ...
    def test_explicit_legacy_state_retirement_renames_verified_state_as_backup(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(
                backup.read_bytes(),
                (target_dir / EMOTION_ENGINE_STATE_PATH).read_bytes(),
            )
    ...
    def test_legacy_state_retirement_refuses_an_existing_backup(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            target_dir = Path(tmpdir)
            legacy = target_dir / ".emotion-engine" / "codex-state.json"
            backup = legacy.with_name(legacy.name + ".bak")
            legacy.parent.mkdir(parents=True)
            legacy.write_text('{"session_count":1}\n', encoding="utf-8")
    ...
    def test_legacy_state_retirement_refuses_diverged_state(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            target_dir = Path(tmpdir)
            canonical = target_dir / EMOTION_ENGINE_STATE_PATH
            legacy = target_dir / ".emotion-engine" / "codex-state.json"
            canonical.parent.mkdir(parents=True)
            canonical.write_text('{"session_count":2}\n', encoding="utf-8")
            legacy.write_text('{"session_count":1}\n', encoding="utf-8")

            with self.assertRaisesRegex(
                PackwrightValidationError,
                "differs from canonical state",
            ):
            ...
            self.assertFalse(
                legacy.with_name(legacy.name + ".bak").exists()
            )

    def test_emotion_engine_to_pi_reports_no_destination_runtime_projection(self):
        resolved = resolve_mechanism(load_mechanism(MECHANISM_PATH))
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            source = root / "emotion-engine"
            ...
            exclusions = [
                item
                for item in report["changes"]["excluded"]
                if item["id"] == "source_emotion_engine_projection_excluded"
            ]
            self.assertTrue(exclusions)
            self.assertEqual(
                {item["reason"] for item in exclusions},
                {
                    "pi has no Emotion Engine runtime projection; "
            ...
            runtime_exclusions = [
                item
                for item in result["runtime_exclusions"]
                if item["id"] == "source_emotion_engine_projection_excluded"
            ]
            self.assertTrue(runtime_exclusions)
            self.assertEqual(
                {item["reason"] for item in runtime_exclusions},
                {
                    "pi has no Emotion Engine runtime projection; "
                    "the source runtime artifact is not copied"
                },
            ...
            self.assertEqual(
                result["emotion_engine_state"]["status"],
                "snapshot_inert",
            )
    ...
    def test_emotion_engine_refuses_ambiguous_live_state_before_writing_target(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("canonical and legacy state candidates differ", str(raised.exception))
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for step, (adapter, skill_path, config_path) in enumerate((
                ...
                with self.subTest(adapter=adapter):
                    ...
                    self.assertEqual((target / EMOTION_ENGINE_STATE_PATH).read_bytes(), expected_state)
...
""",
            ...
            state_path = source_target_dir / EMOTION_ENGINE_STATE_PATH
            state = json.loads(state_path.read_text(encoding="utf-8"))
            ...
            self.assertIn(EMOTION_ENGINE_STATE_PATH, result["state_snapshots"])
            ...
            migrated_state = json.loads(
                (cursor_target_dir / EMOTION_ENGINE_STATE_PATH).read_text(encoding="utf-8")
            )
    ...
    def test_refresh_emotion_engine_updates_runtime_without_resetting_state(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            target_helper = target_dir / EMOTION_ENGINE_RUNTIME_ROOT / "scripts" / "emotion_engine_utils.py"
            unmanaged_note = target_dir / EMOTION_ENGINE_RUNTIME_ROOT / "scripts" / "local-note.txt"
            unmanaged_note.write_text("preserve local sidecar note\n", encoding="utf-8")
            target_helper.write_text("# stale installed helper\n", encoding="utf-8")
            source_helper_text = (
                (sidecar_source / "scripts" / "emotion_engine_utils.py").read_text(encoding="utf-8")
            ...
            target_manifest = json.loads((target_dir / "manifest.json").read_text(encoding="utf-8"))
            self.assertTrue(target_manifest["features"]["emotion_engine"]["installed"])
            self.assertIn(f"{EMOTION_ENGINE_RUNTIME_ROOT}/scripts/emotion_engine_utils.py", target_manifest["artifacts"])
            self.assertIn(f"{EMOTION_ENGINE_RUNTIME_ROOT}/scripts/emotion_engine_mcp.py", target_manifest["artifacts"])
            self.assertIn(f"{EMOTION_ENGINE_RUNTIME_ROOT}/scripts/register_mcp_client.py", target_manifest["artifacts"])
    ...
    def test_doctor_fix_repairs_emotion_engine_sidecar_drift(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            state_file.write_text(json.dumps(state, indent=2, sort_keys=True) + "\n", encoding="utf-8")
            target_helper = target_dir / EMOTION_ENGINE_RUNTIME_ROOT / "scripts" / "emotion_engine_utils.py"
            target_mcp = target_dir / EMOTION_ENGINE_RUNTIME_ROOT / "scripts" / "emotion_engine_mcp.py"
            target_register = target_dir / EMOTION_ENGINE_RUNTIME_ROOT / "scripts" / "register_mcp_client.py"
            target_helper.write_text("# stale installed helper\n", encoding="utf-8")
            target_mcp.unlink()
            ...
            self.assertEqual(
                {issue["id"] for issue in fixed["after_issues"]},
    ...
    def test_rc3_refresh_preserves_v2_until_explicit_verified_migration(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            legacy["_schema"] = "emotion-engine-state/v2"
            for key in ("identity", "capabilities", "session", "session_ledger", "processed_event_ids"):
                legacy.pop(key, None)
            ...
            self.assertEqual(state_file.read_bytes(), legacy_bytes)
            self.assertEqual(
                refreshed["sidecars"]["emotion-engine"]["activation"]["status"],
                "migration_required",
            )
            diagnosed = doctor_target(target_dir)
            self.assertFalse(diagnosed["ok"], diagnosed)
            ...
            dry_run = migrate_emotion_engine_state(target_dir)
            self.assertEqual(dry_run["status"], "migration_ready")
            self.assertFalse(dry_run["applied"])
            self.assertEqual(dry_run["identity"]["character_id"], "atlas")
            self.assertEqual(state_file.read_bytes(), legacy_bytes)
            ...
            self.assertEqual(Path(applied["backup"]).read_bytes(), legacy_bytes)
    ...
    def test_previous_generation_v3_refreshes_then_upgrades_capabilities_transactionally(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            artifact_lock_path = target_dir / LOCK_PATH
            artifact_lock = json.loads(artifact_lock_path.read_text(encoding="utf-8"))
            artifact_lock["artifacts"]["manifest.json"] = hashlib.sha256(
    ...
    def test_failed_emotion_state_verification_rolls_back_v2_and_manifest(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(state_file.read_bytes(), legacy_bytes)
            self.assertEqual((target_dir / "manifest.json").read_bytes(), manifest_before)
            self.assertEqual(result["activation"]["status"], "migration_required")
            diagnosed = doctor_target(target_dir)
            self.assertEqual(
                {issue["id"] for issue in diagnosed["issues"]},
                {
    ...
    def test_codex_lifecycle_bridge_is_idempotent_and_defers_close_until_new_native_session(self):
        resolved = resolve_mechanism(load_mechanism(MECHANISM_PATH))
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            pack_dir = root / "pack"
            ...
            bridge = target_dir / "scripts" / "emotion_engine_lifecycle.py"

            def run_start(session_id, source_name):
                result = subprocess.run(
                    [sys.executable, str(bridge), "codex"],
                    cwd=str(target_dir),
                    input=json.dumps({
            ...
            run_start("native-session-1", "startup")
            run_start("native-session-1", "resume")
            state_file = target_dir / EMOTION_ENGINE_STATE_PATH
            first = json.loads(state_file.read_text(encoding="utf-8"))
            self.assertEqual(first["session_count"], 1)
            self.assertEqual(first["session"]["active_session_id"], "native-session-1")

            run_start("native-session-2", "startup")
            second = json.loads(state_file.read_text(encoding="utf-8"))
            self.assertEqual(second["session_count"], 2)
            self.assertEqual(second["session_ledger"][0]["status"], "closed")
            self.assertEqual(second["session"]["active_session_id"], "native-session-2")
            receipt = json.loads(
                (target_dir / ".packwright" / "activation" / "emotion-engine-lifecycle.json").read_text(encoding="utf-8")
            )
            self.assertEqual(receipt["native_event"], "SessionStart")
            self.assertEqual(receipt["native_session_id"], "native-session-2")
            self.assertEqual(receipt["event_id_source"], "derived_from_native_event_and_session")
            self.assertEqual([item["command"] for item in receipt["operations"]], ["session_end", "session_start"])
            late_before = state_file.read_bytes()
            run_start("native-session-1", "resume")
            self.assertEqual(state_file.read_bytes(), late_before)
            late_receipt = json.loads(
                (target_dir / ".packwright" / "activation" / "emotion-engine-lifecycle.json").read_text(encoding="utf-8")
            )
            self.assertEqual(late_receipt["native_session_id"], "native-session-2")
            _initialize_emotion_engine_mcp(target_dir)
            diagnosed = doctor_target(target_dir)
            self.assertTrue(diagnosed["ok"], diagnosed)
            self.assertNotIn(
                "emotion_engine_lifecycle_unverified",
                {warning["id"] for warning in diagnosed["warnings"]},
            )
            ...
            paused_before = state_file.read_bytes()
            run_start("native-session-3", "startup")
            self.assertEqual(state_file.read_bytes(), paused_before)

    def test_refresh_invalidates_a_running_mcp_projection_nonce(self):
        resolved = resolve_mechanism(load_mechanism(MECHANISM_PATH))
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            pack_dir = root / "pack"
            ...
            try:
                ...
                first_receipt = json.loads(
                    (target_dir / EMOTION_ENGINE_MCP_ACTIVATION_RECEIPT_PATH).read_text(
                        encoding="utf-8"
                    )
                )
                ...
                second_projection = json.loads(
                    (target_dir / EMOTION_ENGINE_PROJECTION_RECEIPT_PATH).read_text(
                        encoding="utf-8"
                    )
                )
                self.assertNotEqual(
                    first_receipt["projection_nonce"],
                    second_projection["projection_nonce"],
                )
                process.stdin.write(
    ...
    def test_managed_mcp_rejects_state_override_and_migration_bypass(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                self.assertIsNone(responses[3]["id"])
    ...
    def test_mcp_holds_target_lock_until_child_response_and_rejects_wrong_id(self):
        resolved = resolve_mechanism(load_mechanism(MECHANISM_PATH))
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            pack_dir = root / "pack"
            ...
            mcp_rel = f"{EMOTION_ENGINE_RUNTIME_ROOT}/scripts/emotion_engine_mcp.py"
            mcp = target_dir / mcp_rel
            mcp.write_text(
                "#!/usr/bin/env python3\n"
                ...
                "    Path('.packwright/test-child-entered').write_text('entered\\n')\n"
                "    time.sleep(0.5)\n"
                "    print(json.dumps({'jsonrpc':'2.0','id':999,'result':{}}), flush=True)\n",
                encoding="utf-8",
            )
            projection_path = target_dir / EMOTION_ENGINE_PROJECTION_RECEIPT_PATH
            projection = json.loads(projection_path.read_text(encoding="utf-8"))
            projection["files"][mcp_rel] = _test_sha256(mcp)
            ...
            try:
                ...
                process.stdin.flush()
                child_entered = target_dir / ".packwright/test-child-entered"
                deadline = time.monotonic() + 3
                while not child_entered.is_file() and time.monotonic() < deadline:
                    time.sleep(0.01)
                self.assertTrue(child_entered.is_file())
                with ThreadPoolExecutor(max_workers=1) as executor:
                    ...
                    time.sleep(0.15)
                    self.assertFalse(refreshed.done())
                    response = json.loads(process.stdout.readline())
                    self.assertEqual(response["id"], 1)
                    self.assertEqual(
                        response["error"]["data"]["reason"],
                        "child_response_id_mismatch",
                    )
                    refreshed.result(timeout=5)
            finally:
                if process.stdin:
                    process.stdin.close()
                process.wait(timeout=5)
    ...
    def test_incomplete_capabilities_fail_closed_across_runtime_writers(self):
        resolved = resolve_mechanism(load_mechanism(MECHANISM_PATH))
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            pack_dir = root / "pack"
            ...
            lifecycle = subprocess.run(
                [sys.executable, str(target_dir / "scripts/emotion_engine_lifecycle.py"), "codex"],
                cwd=str(target_dir),
                input=json.dumps({
                    "hook_event_name": "SessionStart",
                    "session_id": "capability-fuse",
                }),
                check=False,
                capture_output=True,
                text=True,
            ...
            self.assertEqual(mcp.returncode, 7)
            self.assertEqual(
                json.loads(mcp.stdout)["error"]["data"]["reason"],
                "activation_state_verification_failed",
            )
            self.assertEqual(state_file.read_bytes(), before)
            self.assertNotIn(
                "bounded_active_session/v1",
                json.loads(state_file.read_text(encoding="utf-8"))["capabilities"],
            )

    def test_manifest_drift_cannot_be_washed_by_shell_or_mcp_handshake(self):
        resolved = resolve_mechanism(load_mechanism(MECHANISM_PATH))
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            pack_dir = root / "pack"
            ...
            manifest_path = target_dir / "manifest.json"
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            manifest["tampered_after_verification"] = True
            manifest_path.write_text(
                json.dumps(manifest, indent=2, sort_keys=True) + "\n",
                encoding="utf-8",
            )
            manifest_tampered = manifest_path.read_bytes()
            lock_path = target_dir / LOCK_PATH
            ...
            self.assertEqual(shell.returncode, 75)
            self.assertIn("manifest_lock_drift", shell.stderr)
            mcp = subprocess.run(
                ["sh", str(target_dir / "scripts/emotion_engine_mcp.sh")],
                cwd=str(target_dir),
                input='{"jsonrpc":"2.0","id":2,"method":"initialize","params":{}}\n',
                check=False,
                capture_output=True,
                text=True,
            )
            self.assertNotEqual(mcp.returncode, 0)
            self.assertEqual(
                json.loads(mcp.stdout)["error"]["data"]["reason"],
                "manifest_lock_drift",
            )
            self.assertEqual(state_file.read_bytes(), state_before)
            self.assertEqual(manifest_path.read_bytes(), manifest_tampered)
    ...
    def test_concurrent_refreshes_serialize_complete_writer_cohorts(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            pack_dir = root / "pack"
            target_dir = root / "target"
            source_a = root / "emotion-a"
            source_b = root / "emotion-b"
            _write_pack(compile_to_codex_pack(resolved), pack_dir)
            _write_fake_emotion_engine_sidecar(source_a)
            shutil.copytree(source_a, source_b)
            helper_b = source_b / "scripts/emotion_engine_utils.py"
            helper_b.write_text(
                helper_b.read_text(encoding="utf-8") + "\n# source-b cohort\n",
                encoding="utf-8",
            )
            install_pack(
                pack_dir,
                target_dir,
                include_emotion_engine=True,
                emotion_engine_source=source_a,
            )

            with ThreadPoolExecutor(max_workers=2) as executor:
                results = list(
                    executor.map(
                        lambda source: refresh_emotion_engine(
                            target_dir,
                            emotion_engine_source=source,
                        ),
                        (source_a, source_b),
            ...
            feature = manifest["features"]["emotion_engine"]
            self.assertEqual(feature["source_digest"], receipt["source_digest"])
            self.assertEqual(feature["projection_nonce"], receipt["projection_nonce"])
            for rel_path, expected_hash in receipt["files"].items():
                self.assertEqual(_test_sha256(target_dir / rel_path), expected_hash)
            _initialize_emotion_engine_mcp(target_dir)
            self.assertTrue(doctor_target(target_dir)["ok"])

    def test_refresh_exception_restores_complete_writer_cohort(self):
        resolved = resolve_mechanism(load_mechanism(MECHANISM_PATH))
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            pack_dir = root / "pack"
            ...
            tracked = [
                ...
                target_dir / EMOTION_ENGINE_RUNTIME_ROOT / "scripts" / "emotion_engine_utils.py",
            ]
            before = {path: path.read_bytes() for path in tracked if path.is_file()}
            source_helper = source / "scripts" / "emotion_engine_utils.py"
            source_helper.write_text(
                source_helper.read_text(encoding="utf-8") + "\n# replacement cohort\n",
                encoding="utf-8",
            )
            ...
            with patch(
                ...
                with self.assertRaisesRegex(RuntimeError, "manifest commit failed"):
                    refresh_emotion_engine(
                        target_dir,
                        emotion_engine_source=source,
                    )
    ...
    def test_pending_projection_fuses_shell_and_lifecycle_writers(self):
        resolved = resolve_mechanism(load_mechanism(MECHANISM_PATH))
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            pack_dir = root / "pack"
            ...
            pending.write_text('{"status":"projecting"}\n', encoding="utf-8")
    ...
    def test_in_progress_migration_journal_fuses_every_writer_gateway(self):
        resolved = resolve_mechanism(load_mechanism(MECHANISM_PATH))
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            pack_dir = root / "pack"
            ...
            self.assertEqual(shell.returncode, 75)
            self.assertIn("migration_in_progress", shell.stderr)
            ...
            with self.assertRaisesRegex(PackwrightValidationError, "still in progress"):
                refresh_emotion_engine(target_dir, emotion_engine_source=source)
            with self.assertRaisesRegex(PackwrightValidationError, "still in progress"):
                install_pack(
                    pack_dir,
    ...
    def test_migration_manifest_exception_restores_state_manifest_and_lock(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            pack = embed_pack_metadata(
                compile_to_codex_pack(resolved),
                resolved,
                score_mechanism(resolved, compile_to_codex_pack(resolved), adapter="codex"),
            )
            _write_pack(pack, pack_dir)
            _write_fake_emotion_engine_sidecar(source)
            install_pack(
            ...
            with patch(
                ...
                side_effect=RuntimeError("forced lock update failure"),
            ):
                with self.assertRaisesRegex(RuntimeError, "forced lock update failure"):
                    migrate_emotion_engine_state(target_dir, apply=True)

            self.assertEqual(state_file.read_bytes(), state_before)
            self.assertEqual((target_dir / "manifest.json").read_bytes(), manifest_before)
            self.assertEqual((target_dir / LOCK_PATH).read_bytes(), lock_before)
            journal = json.loads(
                (target_dir / EMOTION_ENGINE_MIGRATION_JOURNAL_PATH).read_text(
                    encoding="utf-8"
                )
            )
    ...
    def test_placeholder_identity_requires_explicit_migration_binding(self):
        resolved = resolve_mechanism(load_mechanism(MECHANISM_PATH))
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            pack_dir = root / "pack"
            ...
            manifest_path = target_dir / "manifest.json"
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            manifest["character"]["slug"] = "character"
            manifest["sidecars"]["emotion-engine"]["identity"] = {
                "character_id": "character",
                "relationship_id": "character:primary-user",
            }
            manifest_path.write_text(
                json.dumps(manifest, indent=2, sort_keys=True) + "\n",
                encoding="utf-8",
            ...
            planned = migrate_emotion_engine_state(target_dir)
            self.assertEqual(planned["status"], "identity_confirmation_required")
            with self.assertRaises(PackwrightValidationError):
                migrate_emotion_engine_state(target_dir, apply=True)

    def test_state_and_backup_symlink_escapes_are_rejected_without_external_writes(self):
        resolved = resolve_mechanism(load_mechanism(MECHANISM_PATH))
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            pack_dir = root / "pack"
            ...
            target_dir = root / "state-target"
            outside_state = root / "outside-state"
            outside_state.mkdir()
            (target_dir / ".emotion-engine").mkdir(parents=True)
            (target_dir / ".emotion-engine/generations").symlink_to(
                outside_state,
                target_is_directory=True,
            )
            with self.assertRaisesRegex(PackwrightValidationError, "symlink"):
                install_pack(
                    pack_dir,
                    target_dir,
                    include_emotion_engine=True,
            ...
            self.assertEqual(list(outside_state.iterdir()), [])

            race_target = root / "planned-state-target"
            race_plan = plan_install(
                pack_dir,
                race_target,
                include_emotion_engine=True,
                emotion_engine_source=source,
            )
            outside_race = root / "outside-planned-state"
            outside_race.mkdir()
            (race_target / ".emotion-engine").mkdir(parents=True)
            (race_target / ".emotion-engine/generations").symlink_to(
                outside_race,
                target_is_directory=True,
            )
            with self.assertRaisesRegex(PackwrightValidationError, "symlink"):
                apply_install(race_plan)
            ...
            state_file = backup_target / EMOTION_ENGINE_STATE_PATH
            _convert_emotion_state_to_v2(state_file)
            outside_backup = root / "outside-backup"
            outside_backup.mkdir()
            (backup_target / ".emotion-engine/backups").symlink_to(
                outside_backup,
                target_is_directory=True,
            )
            state_before = state_file.read_bytes()
            with self.assertRaisesRegex(PackwrightValidationError, "symlink"):
                migrate_emotion_engine_state(backup_target, apply=True)
            self.assertEqual(state_file.read_bytes(), state_before)
            self.assertEqual(list(outside_backup.iterdir()), [])

    def test_legacy_writer_path_cannot_downgrade_generation_state(self):
        resolved = resolve_mechanism(load_mechanism(MECHANISM_PATH))
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            pack_dir = root / "pack"
            ...
            _write_pack(compile_to_codex_pack(resolved), pack_dir)
            _write_fake_emotion_engine_sidecar(source)
            legacy_path = target_dir / ".emotion-engine/state.json"
            legacy_path.parent.mkdir(parents=True, exist_ok=True)
            legacy_bytes = b'{"_schema":"emotion-engine-state/v2","session_count":4}\n'
            ...
            generation_state = target_dir / EMOTION_ENGINE_STATE_PATH
            self.assertNotEqual(generation_state.resolve(), legacy_path.resolve())
            migrated = migrate_emotion_engine_state(target_dir, apply=True)
            self.assertEqual(migrated["status"], "migrated", migrated)
            _initialize_emotion_engine_mcp(target_dir)
            generation_before = generation_state.read_bytes()

            legacy_path.write_bytes(
                b'{"_schema":"emotion-engine-state/v2","session_count":99}\n'
            )
            self.assertEqual(generation_state.read_bytes(), generation_before)
            self.assertEqual(
                json.loads(generation_state.read_text(encoding="utf-8"))["_schema"],
                "emotion-engine-state/v3",
            )

    def test_migration_rejects_newer_legacy_write_after_canonical_snapshot(self):
        resolved = resolve_mechanism(load_mechanism(MECHANISM_PATH))
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            pack_dir = root / "pack"
            ...
            legacy_path.write_text(
                '{"_schema":"emotion-engine-state/v2","session_count":4}\n',
                encoding="utf-8",
            )
            install_pack(
                pack_dir,
            ...
            legacy_path.write_text(
                '{"_schema":"emotion-engine-state/v2","session_count":5,"latest":"legacy-writer"}\n',
                encoding="utf-8",
            )

            with self.assertRaisesRegex(
                PackwrightValidationError,
                "canonical and legacy state candidates differ",
            ):
                migrate_emotion_engine_state(target_dir, apply=True)
    ...
    def test_migrated_canonical_v3_wins_over_legacy_v2_and_can_retire_by_journal(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            lineage_id = json.loads(canonical.read_text(encoding="utf-8"))[
                "packwright_migration_lineage"
            ]["lineage_id"]
    ...
    def test_claude_projection_uses_native_upstream_skill_without_codex_rewrite(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            target_dir = root / "target"
            source = root / "emotion-engine"
            _write_pack(compile_to_claude_code_pack(resolved), pack_dir)
            _write_fake_emotion_engine_sidecar(source)
            native = source / "integrations/claude-skill/emotion-engine/SKILL.md"
            native.parent.mkdir(parents=True, exist_ok=True)
            native.write_text(
                "# Emotion Engine For Claude\n\n"
                "Use CLAUDE_EMOTION_STATE and scripts/claude_emotion.sh. "
                "Run register_mcp_client.py claude, settle_trust, require event_id, "
                "and use Pause/Resume controls.\n",
                encoding="utf-8",
            )
            install_pack(
                pack_dir,
            ...
            skill = (target_dir / ".claude/skills/emotion-engine/SKILL.md").read_text(
                encoding="utf-8"
            )
            self.assertIn("CLAUDE_EMOTION_STATE", skill)
            self.assertIn("register_mcp_client.py claude", skill)
            self.assertIn("record_policy", skill)
            self.assertIn("`paused` mode, do not record, decay, or modulate", skill)
            self.assertNotIn("CODEX_", skill)
            self.assertNotIn("register_mcp_client.py codex", skill)

    def test_doctor_fix_upgrades_complete_v1_projection_but_leaves_v2_state_read_only(self):
        resolved = resolve_mechanism(load_mechanism(MECHANISM_PATH))
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            pack_dir = root / "pack"
            ...
            manifest_path = target_dir / "manifest.json"
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            manifest["features"]["emotion_engine"] = {
                "installed": True,
                "adapter": "codex",
                "version": "1.0.0",
                "mode": "light",
                "default_mode": "light",
                "source_digest": "old-v1",
                "mcp_config_path": ".codex/config.toml",
            }
            manifest["sidecars"]["emotion-engine"] = {
                "version": "1.0.0",
                "mode": "light",
                "state_file": EMOTION_ENGINE_STATE_PATH,
                "skill_path": ".agents/skills/emotion-engine/SKILL.md",
                "source_digest": "old-v1",
            }
            manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
            artifact_lock_path = target_dir / LOCK_PATH
            artifact_lock = json.loads(artifact_lock_path.read_text(encoding="utf-8"))
            artifact_lock["artifacts"].pop("manifest.json", None)
            artifact_lock_path.write_text(
                json.dumps(artifact_lock, indent=2, sort_keys=True) + "\n",
                encoding="utf-8",
            )
            helper = target_dir / EMOTION_ENGINE_RUNTIME_ROOT / "scripts/emotion_engine_utils.py"
            helper.write_text("# old v1 helper\n", encoding="utf-8")
            ...
            self.assertIn(
                "emotion_engine_state_migration_required",
                {issue["id"] for issue in repaired["issues"]},
            )
            upgraded_manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            feature = upgraded_manifest["features"]["emotion_engine"]
            self.assertEqual(feature["version"], EMOTION_ENGINE_VERSION)
            self.assertEqual(feature["state_schema"], "emotion-engine-state/v3")
            self.assertEqual(feature["activation"]["status"], "migration_required")
            self.assertIn(f'ENGINE_VERSION = "{EMOTION_ENGINE_VERSION}"', helper.read_text(encoding="utf-8"))
            self.assertTrue((target_dir / "scripts/emotion_engine_lifecycle.py").is_file())
    ...
    def test_cli_install_writes_codex_working_directory(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue((target_dir / ".agents" / "skills" / "emotion-engine" / "SKILL.md").exists())
            self.assertTrue((target_dir / EMOTION_ENGINE_RUNTIME_ROOT / "scripts" / "emotion_engine_mcp.py").exists())
            self.assertTrue((target_dir / EMOTION_ENGINE_RUNTIME_ROOT / "scripts" / "register_mcp_client.py").exists())
            self.assertTrue((target_dir / EMOTION_ENGINE_STATE_PATH).exists())
            target_manifest = json.loads((target_dir / "manifest.json").read_text(encoding="utf-8"))
            self.assertTrue(target_manifest["features"]["emotion_engine"]["installed"])
            self.assertEqual(target_manifest["features"]["emotion_engine"]["mode"], "light")
            ...
            target_helper = target_dir / EMOTION_ENGINE_RUNTIME_ROOT / "scripts" / "emotion_engine_utils.py"
            target_helper.write_text("# stale installed helper\n", encoding="utf-8")
            source_helper = sidecar_source / "scripts" / "emotion_engine_utils.py"
            source_helper.write_text(
                source_helper.read_text(encoding="utf-8")
                + "\n\ndef state_file_lock(path):\n    return path\n",
                encoding="utf-8",
...
def _write_fake_emotion_engine_sidecar(source_dir):
    ...
    (source_dir / "SKILL.md").write_text(
        "# Emotion Engine For Codex\n\nUse this skill for lightweight emotional continuity. Run record_policy before record_turn. Every mutation requires an event_id and bound identity. Run settle_trust at session close. In paused mode do not record or modulate turns.\n\n## Runtime Modes And Record Policy\n",
        encoding="utf-8",
    )
...
STATE_CAPABILITIES = [
    "state_identity/v1", "structured_record_policy/v1", "session_idempotency/v1",
    "trust_evidence/v1", "behavior_audit/v1", "repair_plan/v1",
...
def default_state(args):
    return {
        ...
        "identity": {
            "state_id": "test-state",
            "character_id": option(args, "--character-id"),
            "relationship_id": option(args, "--relationship-id"),
            "status": "bound",
        },
        ...
        "emotion": {"pleasure": 0.0, "arousal": 0.3, "dominance": 0.5},
        "affective_pulse": {"P": 0.0, "A": 0.0, "D": 0.0, "intensity": 0.0, "label": "none", "source": "default", "created_at": None},
        "personality_baseline": {"pleasure": 0.0, "arousal": 0.3, "dominance": 0.5},
        "character_profile": {"source": "default", "description": "warm, steady", "interpretation": "warm, steady", "traits": ["warm", "steady"]},
        "trust": 0.1, "trust_anchor": 0.1, "session_count": 0, "total_turns": 0,
        "last_interaction_iso": None, "emotion_trajectory": [], "emotion_log": [],
        "trust_history": [], "trust_evidence": [], "trust_settlements": [],
        "processed_event_ids": [], "session_ledger": [],
        "session": {"active_session_id": None, "last_session_id": None, "status": "idle", "opened_at": None, "closed_at": None, "settled_at": None},
        "log_limit": 200,
    }
...
def missing_state_capabilities(state):
    return [item for item in STATE_CAPABILITIES if item not in state.get("capabilities", [])]

def require_current_state_capabilities(state):
    missing = missing_state_capabilities(state)
    if missing:
        raise ValueError("capability_upgrade_required")
    return state
...
def main():
    ...
    if command == "init":
        ...
        save(path, state)
        print(json.dumps({"ok": True, "engine_version": ENGINE_VERSION, "schema": STATE_SCHEMA, "identity_status": "bound", "capabilities": STATE_CAPABILITIES}))
        return 0
    state = json.loads(path.read_text(encoding="utf-8"))
    if command == "migrate_state":
        migrated = default_state(args)
        for key, value in state.items():
            if key not in {"_schema", "identity", "capabilities", "session", "session_ledger", "processed_event_ids"}:
                migrated[key] = value
        result = {"status": "migration_ready", "from_schema": LEGACY_STATE_SCHEMA, "to_schema": STATE_SCHEMA, "dry_run": "--apply" not in args}
        if "--apply" in args:
    ...
    if command == "activation_check":
        ...
        missing = [item for item in STATE_CAPABILITIES if item not in state.get("capabilities", [])]
        if missing:
            print(json.dumps({"ok": False, "engine_version": ENGINE_VERSION, "status": "capability_upgrade_required", "missing_capabilities": missing}))
            return 2
        print(json.dumps({"ok": True, "engine_version": ENGINE_VERSION, "status": "ready", "schema": STATE_SCHEMA, "capabilities": STATE_CAPABILITIES}))
        return 0
    if command == "audit_state":
        if os.environ.get("PACKWRIGHT_TEST_FAIL_EMOTION_AUDIT") == "1":
    ...
    if command == "configure":
        ...
        state["character_profile"]["description"] = description
        state["character_profile"]["interpretation"] = state["character_profile"]["description"]
        if any(word in description.lower() for word in ("warm", "playful", "intimate", "亲密", "活泼")):
            state["personality_baseline"] = {"pleasure": 0.42, "arousal": 0.35, "dominance": 0.6}
            state["emotion"] = dict(state["personality_baseline"])
        if any(word in description.lower() for word in ("playful", "intimate", "companion", "亲密", "活泼")):
            state["volatility_profile"] = "expressive"
        save(path, state)
        print(json.dumps({"ok": True, "status": "configured"}))
        return 0
    if command == "pause":
        state["enabled"] = False
        state["runtime_mode"] = "paused"
        save(path, state)
        print(json.dumps({"ok": True, "enabled": False, "runtime_mode": "paused"}))
        return 0
    if command == "resume":
        state["enabled"] = True
        state["runtime_mode"] = option(args, "--mode") or "light"
        save(path, state)
        print(json.dumps({"ok": True, "enabled": True, "runtime_mode": state["runtime_mode"]}))
        return 0
    if command == "session_start":
        session_id = option(args, "--session-id")
        event_id = option(args, "--event-id")
        active = state["session"].get("active_session_id")
        if any(item.get("session_id") == session_id for item in state["session_ledger"]):
            status = "already_active" if active == session_id else "already_closed"
        elif active and active != session_id:
            status = "active_session_conflict"
        else:
            status = "started"
            state["session_count"] += 1
            state["session"] = {"active_session_id": session_id, "last_session_id": session_id, "status": "active", "opened_at": "now", "closed_at": None, "settled_at": None}
            state["session_ledger"].append({"session_id": session_id, "status": "active"})
            state["processed_event_ids"].append(event_id)
            save(path, state)
        print(json.dumps({"status": status, "session_id": session_id}))
        return 0
    if command == "session_end":
        session_id = option(args, "--session-id")
        if state["session"].get("active_session_id") != session_id:
            status = "no_active_session"
        else:
            status = "closed"
            state["session"]["active_session_id"] = None
            state["session"]["status"] = "closed"
            for item in state["session_ledger"]:
                if item.get("session_id") == session_id:
                    item["status"] = "closed"
            save(path, state)
        print(json.dumps({"status": status, "session_id": session_id}))
        return 0
    print(json.dumps({"ok": True, "command": command, "state_file": str(path), "engine_version": ENGINE_VERSION, "schema": state.get("_schema")}))
    return 0
...
''',
    ...
    (source_dir / "emotion-state-template.json").write_text(
        '{"_schema": "emotion-engine-state/v3", "identity": {"status": "unbound"}, "capabilities": []}\n',
        encoding="utf-8",
    )
...
def _initialize_emotion_engine_mcp(target_dir):
    completed = subprocess.run(
        ["sh", str(Path(target_dir) / "scripts" / "emotion_engine_mcp.sh")],
        cwd=str(target_dir),
        input='{"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}\n',
        check=False,
        capture_output=True,
    ...
    if payload.get("result", {}).get("serverInfo", {}).get("version") != EMOTION_ENGINE_VERSION:
        raise AssertionError(payload)
    return payload


def _convert_emotion_state_to_v2(state_file):
    state = json.loads(Path(state_file).read_text(encoding="utf-8"))
    state["_schema"] = "emotion-engine-state/v2"
    for key in (
        "identity",
        "capabilities",
        "session",
